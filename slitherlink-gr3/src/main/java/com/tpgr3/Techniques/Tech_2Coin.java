...
public class Tech_2Coin implements Techniques {
    ...
    public boolean estApplicable(Grille grille) {
        for (int y = 1; y < grille.hauteur; y += 2) {
            for (int x = 1; x < grille.largeur; x += 2) {
                ...
                if (c != null && c.getValeur() == 2 && estDansUnCoin(x, y, grille)) {
                    List<Slot> slots = grille.getSlotsAdjacentsCase(x, y);
                    System.out.printf("[OK] Tech_2Coin: Case (x=%d,y=%d) dans un coin. Slots adjacents: %d%n", x, y, slots.size());
                    return true;
                }
            }
        }
