...
public class Tech_0And3Adjacent implements Techniques {
    ...
    public List<int[]> getSlotsToMark(Grille grille) {
        ...
        int[] savePos0 = position0;
        List<int[]> saveCroix = new ArrayList<>(slotsCroix);
        List<int[]> saveBaton = new ArrayList<>(slotsBaton);
        
        // Vérifier si applicable
        if (!estApplicable(grille)) {
            // Restaurer l'état et retourner vide
            position3 = savePos3;
            position0 = savePos0;
            slotsCroix = new ArrayList<>(saveCroix);
            slotsBaton = new ArrayList<>(saveBaton);
            return new ArrayList<>();
        }
        
        // À ce stade, slotsCroix et slotsBaton sont remplis par estApplicable et buildCoordinates
        List<int[]> resultat = new ArrayList<>();
        
        // Ajouter toutes les coordonnées
